...
// для Android нужно включить анимации LayoutAnimation
if (
  Platform.OS === "android" &&
...
const LANG_LABELS: Record<PortraitLang, string> = {
  ru: "Рус",
  kz: "Қаз",
  tr: "Türk",
  en: "Eng",
...
const PortraitList = ({ theme, systemLang }: { theme: Theme; systemLang: PortraitLang }) => {
  ...
  const renderLangSwitcher = () => {
    const langs: PortraitLang[] = ["ru", "kz", "tr", "en"];
            ...
            >
              <Text
                style={[
                  styles.langChipText,
                  {
                    color: isActive ? theme.title : theme.subtitle,
                  },
                ]}
              >
                {LANG_LABELS[l]}
  ...
  return (
    ...
    >
      ...
      {allPortraits.map((item) => {
        ...
        // Превью данных в закрытом состоянии
        const preview =
          item.data.length > 90
